	...
	function($rootScope,$location) {
		...
		var controller = function($scope,$element) {
			...
			$scope.getVotes = function(item){
				...
				item.upVotes = 0;
				// get votes 
				var query = ["SELECT * FROM vote WHERE "+$scope.item_id_name+"="+item[$scope.item_id_name]+" AND channel='"+item.channel.address+"'"];
			...
			$scope.voteUp = function(item){
				...
				Page.cmd("fileGet", { "inner_path": inner_path, "required": false },function(data) {
					...
					// vote item id
					vote[$scope.item_id_name] = item[$scope.item_id_name];
					// user
