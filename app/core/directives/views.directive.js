	...
	function() {
		...
		var controller = function($scope,$element) {
			...
			$scope.getViews = function(item){
				...
				item.views = [];
				// get views
				var query = ["SELECT * FROM view WHERE "+$scope.item_id_name+"="+item[$scope.item_id_name]+" AND channel='"+item.channel.address+"' ORDER BY date_added"];
			...
			$scope.createView = function(item){
				...
				Page.cmd("fileGet", { "inner_path": inner_path, "required": false },function(data) {
					...
					// view item id
					view[$scope.item_id_name] = item[$scope.item_id_name];
					// view id
