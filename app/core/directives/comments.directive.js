	...
	function($rootScope,$sce,$location) {
		...
		var controller = function($scope,$element) {
			...
			$scope.getComments = function(item){
				$scope.comments = [];
				var query = ["SELECT * FROM comment WHERE "+$scope.item_id_name+"="+item[$scope.item_id_name]+" AND channel='"+item.channel.address+"' ORDER BY date_added"];
			...
			$scope.postComment = function(comment,item){
				...
				Page.cmd("fileGet", { "inner_path": inner_path, "required": false },function(data) {
					...
					// item id
					comment[$scope.item_id_name] = item[$scope.item_id_name];
					// user
